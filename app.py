...
@app.route("/buy/[int:id](int:id)", methods=["GET", "POST"])
def buy(id):
conn = db()
...
@app.route("/edit-product/[int:id](int:id)", methods=["GET", "POST"])
def edit_product(id):
conn = db()
...
@app.route("/delete-product/[int:id](int:id)")
...
@app.route("/confirm-order/[int:id](int:id)")
